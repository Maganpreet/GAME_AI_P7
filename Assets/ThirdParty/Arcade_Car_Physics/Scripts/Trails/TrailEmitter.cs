...
namespace VehicleBehaviour.Trails
{
	...
	public class TrailEmitter : MonoBehaviour {
		...
		void Update () 
		{
			...
			if (!trailing && wheel.isGrounded && (Mathf.Abs(hit.sidewaysSlip) > 0.4f || Mathf.Abs(hit.forwardSlip) > 0.98f)) 
			{
				trailing = true;
				NewTrail ();
			} else if (trailing && (!wheel.isGrounded || (Mathf.Abs(hit.sidewaysSlip) < 0.2f && Mathf.Abs(hit.forwardSlip) < 0.98f))) 
			{
				trailing = false;
				EndTrail ();
			}
