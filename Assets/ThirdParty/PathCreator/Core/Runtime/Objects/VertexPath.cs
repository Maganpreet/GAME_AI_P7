...
namespace PathCreation {
    ...
    public class VertexPath {
        ...
        public int GetPreviousSegmentIndexAtDistance(float dst, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Stop)
        {
            float t = dst / length;
        ...
        public int GetBezierSegmentIndexAtDistance(float dst, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Stop)
        {
            float t = dst / length;
        ...
        /// Gets point on path based on distance travelled.
        public System.Tuple<Vector3, Vector3> GetPointAndDirAtDistance(float dst, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Stop)
        {
            float t = dst / length;
            var tup = new System.Tuple<Vector3, Vector3>(
                GetPointAtTime(t, endOfPathInstruction),
                GetDirection(t, endOfPathInstruction)
                );
        ...
        public int GetPreviousSegmentIndexAtTime(float t, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Stop)
        {
        ...
        public int GetBezierSegmentIndexAtTime(float t, EndOfPathInstruction endOfPathInstruction = EndOfPathInstruction.Stop)
        {
            var data = CalculatePercentOnPathData(t, endOfPathInstruction);
        ...
        public Vector3 GetClosestPointDirectionOnPath(Vector3 worldPoint)
        {
            Vector3 localPoint = MathUtility.InverseTransformPoint(worldPoint, transform, space);
            TimeOnPathData data = CalculateClosestPointOnPathData(localPoint);
        ...
        public System.Tuple<Vector3,Vector3> GetClosestPointAndDirectionOnPath(Vector3 worldPoint)
        {
            // Transform the provided worldPoint into VertexPath local-space.
            // This allows to do math on the localPoint's, thus avoiding the need to
            // transform each local vertexpath point into world space via GetPoint.
            ...
            var tup = new System.Tuple<Vector3, Vector3>(
                MathUtility.TransformPoint(localResult, transform, space),
                MathUtility.TransformDirection(dir, transform, space)
                );
