...
namespace PathCreation.Examples {
    public class RoadMeshCreator : PathSceneTool {
        ...
        [Tooltip("If true, only the first N Bézier segments are rendered.")]
        public bool limitToFirstBezierSegments = false;

        [Min(1)]
        [Tooltip("Number of initial Bézier segments to include when limiting is enabled.")]
        public int visibleBezierSegments = 1;
        ...
        Mesh CreateRoadMesh()
        {
            // Determine how many polyline vertices to include, optionally cutting at a Bézier segment boundary.
            int includedPointCount = path.NumPoints;
            if (limitToFirstBezierSegments)
            {
                // Compute the vertex index where segment 'visibleBezierSegments' begins, and cut just before it.
                // If the boundary is out of range, include all vertices.
                int boundaryAnchorIndex = Mathf.Max(0, visibleBezierSegments); // segment N starts at anchor N
                int[] anchorVerts = path.localAnchorVertexIndex;
                if (anchorVerts != null && anchorVerts.Length > 0)
                {
                    if (boundaryAnchorIndex < anchorVerts.Length)
                    {
                        includedPointCount = Mathf.Clamp(anchorVerts[boundaryAnchorIndex], 0, path.NumPoints);
                    }
                    else
                    {
                        includedPointCount = path.NumPoints;
                    }
                }
            }

            // Prevent degenerate meshes
            includedPointCount = Mathf.Clamp(includedPointCount, 0, path.NumPoints);
            bool closeLoopForThisBuild = path.isClosedLoop && (includedPointCount == path.NumPoints);
            if (includedPointCount < 2)
            {
                mesh.Clear();
                return mesh;
            }

            // Allocate geometry sized to the visible slice
            Vector3[] verts = new Vector3[includedPointCount * 8];
            Vector2[] uvs = new Vector2[verts.Length];
            Vector3[] normals = new Vector3[verts.Length];

            int numTris = 2 * (includedPointCount - 1) + (closeLoopForThisBuild ? 2 : 0);
            ...
            if (path.localAnchorVertexIndex != null && path.localAnchorVertexIndex.Length > (BezierSegmentReferenceControlPoint - 1))
            {
                int safeAnchorPrev = Mathf.Min(BezierSegmentReferenceControlPoint - 1, path.localAnchorVertexIndex.Length - 1);
                int safeVertexPrev = Mathf.Clamp(path.localAnchorVertexIndex[safeAnchorPrev], 0, path.cumulativeLengthAtEachVertex.Length - 1);
                prevSegUpdatedOffset = (path.cumulativeLengthAtEachVertex[safeVertexPrev] * texturePerMeters) % 1f;
            }
            ...
            for (int i = 0; i < includedPointCount; i++) {
                ...
                if (i < includedPointCount - 1 || closeLoopForThisBuild) {
            ...
            if (path.localAnchorVertexIndex != null && path.localAnchorVertexIndex.Length > BezierSegmentReferenceControlPoint)
            {
                int safeAnchor = Mathf.Min(BezierSegmentReferenceControlPoint, path.localAnchorVertexIndex.Length - 1);
                int safeVertex = Mathf.Clamp(path.localAnchorVertexIndex[safeAnchor], 0, path.cumulativeLengthAtEachVertex.Length - 1);
                prevTextureOffset = (textureOffset + path.cumulativeLengthAtEachVertex[safeVertex] * texturePerMeters) % 1f;
            }
        ...
        void AssignMaterials () {
            if (roadMaterial != null && undersideMaterial != null) {
                ...
                if (meshRenderer.sharedMaterials != null && meshRenderer.sharedMaterials.Length > 0 && meshRenderer.sharedMaterials[0] != null)
                {
